...
> 版本：v0.1.0
> 状态：Sprint 3 执行中（AS-8 授权用量 3/4，余 1 次），FUNC-5 完成，FUNC-7 待开工
...
- `last_run_status`：`FUNC-5 完成（format-smoke 11/11 PASS，FUNC-3/4 回归 28/28 + 13/13 保持，build 通过）；FUNC-7 待开工`
...
- `task_status`：`pending`
- `task_progress`：`FUNC-3（28/28）、FUNC-4（13/13）、FUNC-5（11/11）已闭环（decision-016/017/018）；Sprint 3 执行顺序：FUNC-5 -> FUNC-7 -> FUNC-8 -> FUNC-6`
- `next_action`：`FUNC-7（搜索替换）-> FUNC-8 -> FUNC-6 -> Sprint 3 收口`
